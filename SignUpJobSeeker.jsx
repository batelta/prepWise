...
const SignUpJobSeeker = ({ navigation }) => {
  ...
  /*const uploadResumeFile = async (userId, fileUri) => {
    const formData = new FormData();

    // הוספת הקובץ כ- multipart
    formData.append("file", {
      uri: fileUri,
      //type: "application/pdf", // שימי לב להתאים את הסוג לקובץ שלך
      name: "resume.pdf", // שימי לב, יכול להיות גם file.name אם יש
    });
  ...
  const handleLinkedInLinkChange = (text) => {
    ...
    if (!text.trim()) {
      ...
      !/^(https?:\/\/)?(www\.)?linkedin\.com\/in\/[a-zA-Z0-9\-_]+$/.test(text)
    ) {
    ...
    try {
      ...
      const API_URL =
        Platform.OS === "web"
          ...
          : "http://192.168.30.157:7137/api/Users";
      ...
      formData.append("CareerField", selectedFields.join(","));
      formData.append("Experience", selectedStatuses[0]);
      formData.append("Picture", base64Image ?? "");
      formData.append("Language", selectedLanguages.join(","));
      formData.append("FacebookLink", FacebookLink);
      formData.append("LinkedInLink", LinkedInLink);
      formData.append("IsMentor", isMentor.toString());
      formData.append("Company", isMentor ? Company : "");
      formData.append("MentoringType", isMentor ? MentoringType : "");
  ...
  const loginAsUser = async (email, password) => {
    try {
      console.log("Sending request to API...");
      const API_URL = "https://localhost:7137/api/Users/SearchUser";
        ...
        <View style={appliedStyles.loginBox}>
          {image ? (
            <View style={appliedStyles.imageAndIconContainer}>
              <Image
                source={{ uri: image }}
                style={appliedStyles.profileImage}
              />
              <TouchableOpacity
                onPress={pickImage}
                style={appliedStyles.cameraButtonAfter}
              >
          ...
          <View style={appliedStyles.rowPair}>
            <View style={appliedStyles.inputBlock}>
              ...
              />
              {FirstNameError ? (
                <Text style={{ color: "#003D5B", marginBottom: "1%" }}>
                  {FirstNameError}
                </Text>
              ) : null}
            </View>
